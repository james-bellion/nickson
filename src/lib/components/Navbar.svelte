...
TODO: add to top of the page and when i scroll down I want it to stay at the top of my viewport so i can click and navigate to each section
 -->
...
<script lang="ts">
	...
	let src = '/logo-no-background.png';
	...
	//   if (targetElement) {
	//     const offset = targetElement.offsetTop - document.body.offsetTop;
	//     window.scrollTo({ top: offset, behavior: 'smooth' });

	//     // Set the active section and update link styles
	...
	function updateNavLinkStyles() {
		...
		const currentLink = document.querySelector(`.nav-link[data-section="${activeSection}"]`);
		if (currentLink) {
	...
	afterUpdate(() => {
		// Ensure that active class is updated when using browser back/forward buttons
		updateNavLinkStyles();
	});
</script>
...
<div class="navbar flex w-full">
	...
	<div class="flex justify-end items-center">
		<nav class="flex space-x-4 pr-10 list-none font-semibold text-xl gap-3">
			<!-- TODO for each ? like a map ? -->
			<li
				on:click={(event) => {
			...
			>
				<a
					class="{path === '/'
						? 'active'
						: ''} cursor-pointer text-white nu-font font-bold inverted-2 py-1 px-4">Home</a
				>
			</li>
			<li
				on:click={(event) => {
			...
			>
				<a
					class="{path === '/about'
						? 'active'
						: ''} cursor-pointer text-white nu-font font-bold inverted-2 py-1 px-4">About</a
				>
			</li>
			<li
				on:click={(event) => {
			...
			>
				<a
					class="{path === '/contact'
						? 'active'
						: ''} cursor-pointer text-white nu-font font-bold inverted-2 py-1 px-4">Contact</a
				>
			</li>
